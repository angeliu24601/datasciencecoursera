## 1. Under the lattice graphics system, what do the primary plotting functions like 
...
## or text to the plot, in addition to customizing axis labels or adding titles. 
## Different plotting systems have different sets of functions for annotating 
## plots in this way.
## Which of the following functions can be used to annotate the panels
## in a multi-panel lattice plot?
## ans: panel.lmline(); panel.abline()
...
## 5. In the lattice system, which of the following functions can be used to 
...
## and wind speed varies across each month. What would be the appropriate code 
## to visualize that using ggplot2?
library(datasets)
library(ggplot2)
data(airquality)
...
## dataset in the ggplot2 package. After loading the ggplot2 package with the library()
## function, I can run
qplot(votes, rating, data = movies)
## How can I modify the the code above to add a smoother to the scatterplot?
qplot(votes, rating, data = movies) + geom_smooth() ## yes
qplot(votes, rating, data = movies, smooth = "loess") ## no
qplot(votes, rating, data = movies) + stats_smooth("loess") ## no
